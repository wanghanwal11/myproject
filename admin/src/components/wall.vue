...
<!--</template>-->
<template>
  <div :style="isMobile? '':{width:colWidth*columnCount+'px',left:'50%',marginLeft: -1*colWidth*columnCount/2 +'px'}"  class="vue-waterfall-easy">
    <div  v-for="(v,i) in imgsArrC">
      <a :href="v.link" :style="{padding:gap/2+'px',width: isMobile ? '' : colWidth+'px'}" class="img-box">
          <div class="img-inner-box">
            <div :style="{width:imgWidthC+'px',height:v.height?v.height+'px':''}" class="img-wraper"><img :src="v.src"/></div>
            <!--<div class="img-info"></div>-->
            <slot :index="i" :value="v"></slot>
            <div v-if="isPreloadingC" :class="{'first-loading':isFirstTIme}" class="loading"></div>
          </div>
        </a>
    </div>
  </div>
